...
export const profileSchema = z.discriminatedUnion("role", [
  ...
  adminStaffProfileSchema,
  hodStaffProfileSchema,
]);

// Profile creation schemas (without ID and timestamps)
const createStudentProfileSchema = studentProfileSchema.omit({
  id: true,
  created_at: true,
  updated_at: true,
});
const createAdminStaffProfileSchema = adminStaffProfileSchema.omit({
  id: true,
  created_at: true,
  updated_at: true,
});
const createHodStaffProfileSchema = hodStaffProfileSchema.omit({
  id: true,
  created_at: true,
  updated_at: true,
});
...
export type AdminStaffProfileInput = z.infer<typeof adminStaffProfileSchema>;
export type HodStaffProfileInput = z.infer<typeof hodStaffProfileSchema>;
